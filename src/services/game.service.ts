import { Prisma, PrismaClient, type GameStatus, type Priority } from "@prisma/client";
...
export async function addGameToCollection(externalId: number) {
  ...
  return prisma.game.create({
    data: {
      ...
      timeToBeatCompletely: ttb?.completely
        ? Math.round(ttb.completely / 60)
        : null
    }
  });
}
...
export async function getUserGames(
  ...
  if (options?.search) {
    ...
    const matchingGames = await prisma.$queryRaw<Array<{ id: string }>>`
      SELECT id FROM "Game"
      WHERE unaccent(title) ILIKE unaccent(${searchPattern})
    `;
    gameFilter.id = { in: matchingGames.map(g => g.id) };
  }

  if (options?.platform) {
    gameFilter.platforms = { has: options.platform };
  }

  if (options?.genre) {
    gameFilter.genres = { has: options.genre };
  }
...
export async function updateGameNotes(
  userId: string,
  gameId: string,
...
export async function updateGamePriority(
  userId: string,
  gameId: string,
...
export async function getDashboard(userId: string) {
  ...
  return {
    ...
    estimatedHoursRemaining:
      Math.round((estimatedHoursRemaining / 60) * 10) / 10
  };
}
...
export async function getWishlistGames(userId: string) {
  return prisma.userGame.findMany({
    ...
    include: {
      game: { select: { title: true, coverUrl: true, genres: true } }
    },
    orderBy: { updatedAt: "desc" }
  });
}
